...
>
> **1. 仅在线成绩上传** — 离线游玩的成绩不会被上传到 Milkoud 的 Rank/Recent，且不参与在线 Reality 计算。
>
> **2. Touch / Keyboard 双端不互通** — Milkoud 为触摸端（手机/平板）和键盘端（PC）各自维护独立的排行榜。同一曲目在两个端的成绩不会互相覆盖，但**本地存档是双端共用的**（同一份 saves.db）。
>
> **3. 旧版成绩** — v4.0 以前的成绩不会被上传到云端，也不会参与新版本Reality计算。
>
...
> [!WARNING]
> 云存档功能需要自行配置以下任一内容：
> - [Re Nya Profiler API](https://renya.mhtlim.top/apikey)的API Key
...
> [!WARNING]
>
> - Milthm游戏内不会自动同步云存档，首次使用时需要手动上传存档。存档内成绩只做为补充和参考，不会参与在线成绩计算。
> - 后续只会走Rank和Recent接口获取在线成绩，Recent接口最多20条最近记录，只有极少数情况会走下载存档更新。下载存档每日最多5次，请合理使用。
>
...
- **支持直接Token绑定**: 可前往[Milkloud官网-创建令牌](https://milkloud.milthm.cn/settings/tokens/new)获取直接Token
